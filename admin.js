...
async function loadData() {
    allStudents = await (await fetch("http://localhost:3000/api/students")).json();
    allCourses = await (await fetch("http://localhost:3000/api/courses")).json();
}
loadData();
...
async function deleteCourse() {
    ...
    try {
        ...
        await fetch(`http://localhost:3000/api/courses/${course._id}`, {
            method: "DELETE",
            headers: { 'Content-Type': 'application/json' }
        });
