...
app.post("/api/courses", async (req, res) => {
  ...
  const result = await coursesCol.insertOne({ title, code ,credits});
...
app.post("/api/students", async (req, res) => {
  const { name, email, password, role, registeredCourses} = req.body;
  if (!name || !password) return res.status(400).json({ error: "name is required" });
  const result = await studentsCol.insertOne({ name, email: email || null, registeredCourses: [], password, role, registeredCourses});
