...
async function loadData() {
    students = await (await fetch("http://localhost:3000/api/students")).json();
    courses = await (await fetch("http://localhost:3000/api/courses")).json();
...
async function addCourse() {
    ...
    await fetch(`http://localhost:3000/api/students/${student._id}/register`, {
        method: 'POST',
...
async function removeCourse() {
    ...
    await fetch(`http://localhost:3000/api/students/${student._id}/unregister`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ courseId: course._id })
    });
